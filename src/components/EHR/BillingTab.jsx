    ...
    return (
        <div className="animate-fade-in" style={{ padding: '2.5rem', background: '#fcfcfd', minHeight: '100vh' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                    ...
                    <div className="card shadow-lg" style={{ padding: '2.5rem', borderRadius: '32px', background: 'white', border: '1px solid #e2e8f0', position: 'sticky', top: '2rem' }}>
                        ...
                        <div style={{ display: 'flex', gap: '0.75rem', marginTop: '1rem' }}>
                            ...
                            <button className="btn btn-secondary" onClick={() => setShowCert(true)} style={{ flex: 1, height: '48px', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px', borderRadius: '14px' }}>
                                <FileText size={18} /> {lang === 'TH' ? 'ใบรับรองแพทย์' : 'Certificate'}
                            </button>
                        </div>
                    </div>
                </div>
            ...
            {showCert && ReactDOM.createPortal(
                <CertModal certData={certData} setCertData={setCertData} patient={patient} language={language} setShowCert={setShowCert} />,
                document.body
            )}
...
const CertModal = ({ certData, setCertData, patient, language, setShowCert }) => {
    ...
    return (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(8px)', zIndex: 99999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div className="card shadow-2xl animate-scale-in" style={{ width: '90%', maxWidth: '500px', padding: '2.5rem', borderRadius: '32px', background: 'white' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem' }}>
                    <h2 style={{ margin: 0, fontSize: '1.5rem', fontWeight: 950 }}>{language === 'TH' ? 'ออกใบรับรองแพทย์' : 'Medical Certificate'}</h2>
                    <button onClick={() => setShowCert(false)} style={{ background: 'none', border: 'none', color: '#94a3b8', cursor: 'pointer' }}><X size={24} /></button>
                </div>
                <div style={{ display: 'grid', gap: '1.5rem' }}>
                    <div>
                        <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 800, color: '#64748b', marginBottom: '8px' }}>Diagnosis</label>
                        <textarea className="form-input" value={certData.diagnosis} onChange={(e) => setCertData({...certData, diagnosis: e.target.value})} placeholder="Patient has dental issue..." style={{ minHeight: '100px', width: '100%' }} />
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
                        <div>
                            <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 800, color: '#64748b', marginBottom: '8px' }}>Rest Days</label>
                            <input type="number" className="form-input" value={certData.days} onChange={(e) => setCertData({...certData, days: e.target.value})} style={{ width: '100%' }} />
                        </div>
                        <div>
                            <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: 800, color: '#64748b', marginBottom: '8px' }}>From Date</label>
                            <input type="date" className="form-input" value={certData.from} onChange={(e) => setCertData({...certData, from: e.target.value})} style={{ width: '100%' }} />
                        </div>
                    </div>
                    <button className="btn btn-primary" onClick={handlePrint} style={{ width: '100%', padding: '1rem', borderRadius: '16px', fontWeight: 900, marginTop: '1rem' }}><Printer size={20} /> PRINT CERTIFICATE</button>
                </div>
            </div>
        </div>
    );
