...
/** รองรับทั้งรหัสเก่า (fill_*) และรหัสจาก settings (filling-1, cleaning, ...) */
const inferChartTargetStatus = (procId) => {
    if (!procId) return null;
    const id = String(procId).toLowerCase();
    if (id.startsWith('fill_') || id.includes('filling')) return 'filled';
    if (id.startsWith('extraction_') || id.includes('extract') || id.includes('wisdom')) return 'extracted';
    if (id.startsWith('rct_') || id.includes('rct') || id.includes('root')) return 'rootCanal';
    if (id.startsWith('crown_') || id.includes('crown') || id.includes('veneer')) return 'crown';
    if (id === 'cleaning' || id.includes('clean') || id.includes('scaling') || id.includes('ขูด')) return 'scaling';
    return null;
};

const procedureAccent = (procId) => {
    const st = inferChartTargetStatus(procId);
    if (st === 'filled') return '#3b82f6';
    if (st === 'extracted') return '#ef4444';
    if (st === 'rootCanal') return '#f59e0b';
    if (st === 'crown') return '#8b5cf6';
    if (st === 'scaling') return '#10b981';
    return '#1e40af';
};
...
const TreatmentPlanTab = ({ patient, language: propsLang, onUpdateToothStatus }) => {
    ...
    const handleAddTreatmentToPlan = (treatmentData) => {
        ...
        const targetStatus = inferChartTargetStatus(procId);

        if (targetStatus && selectedTeeth.length > 0) {
                                    ...
                                    {activePlan.items.map((item, i) => (
                                        <tr key={i} style={{ borderBottom: '1px solid var(--neutral-50)' }}>
                                             <td style={{ 
                                                 padding: '0.75rem 0', 
                                                 fontWeight: 600, 
                                                 color: procedureAccent(item.procedureId),
                                                 display: 'flex', 
                                                 alignItems: 'center', 
                                                 gap: '8px' 
                                             }}>
                                                 <div style={{ 
                                                     ...
                                                     background: procedureAccent(item.procedureId)
                                                 }}></div>
