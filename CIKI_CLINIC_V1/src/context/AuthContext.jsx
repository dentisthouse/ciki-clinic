...
export const AuthProvider = ({ children }) => {
    ...
    const fetchRolePermissions = async () => {
        try {
            // Use local server for permissions
            const { data, error } = await withTimeout(supabaseLocal.from('role_permissions').select('*'), 2500);
            ...
            console.warn('Local permissions fetch took too long or failed, using defaults'); 
        }
    ...
    const fetchStaffData = async (uid, email) => {
        ...
        try {
            // Check profiles table on Local Server
            const { data, error } = await withTimeout(
                supabaseLocal.from('profiles')
                    .select('*')
            ...
            if (data) {
                ...
                // Fallback to Cloud Supabase for profiles if not on local (Transition period)
                const { data: cloudData } = await withTimeout(
                    supabase.from('profiles').select('*').ilike('email', normalizedEmail).maybeSingle(),
                    3000
                );
                if (cloudData) {
                    setStaffRecord({ ...cloudData, role: isOwnerByEmail ? 'owner' : cloudData.role });
                    return;
                }

                // Critical Fallback: Check hardcoded list
    ...
    useEffect(() => {
        ...
        // Listen to Auth changes on Local Server
        const { data: { subscription } } = supabaseLocal.auth.onAuthStateChange(async (_event, session) => {
            const currentUser = session?.user || null;
            setUser(currentUser);
    ...
    const logout = async () => await supabaseLocal.auth.signOut();
    ...
    const value = { 
        ...
        saveRolePermissions: async (updatedPerms) => {
            ...
            const { error } = await supabaseLocal.from('role_permissions').upsert(rows, { onConflict: 'role' });
            return !error;
        }, 
        fetchRolePermissions,
        loading, 
        login: (e, p) => supabaseLocal.auth.signInWithPassword({ email: e, password: p }), 
        logout 
    };
